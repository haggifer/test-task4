...
export default function Exchange(): ReactElement {
  ...
  const { data, error, isLoading } = useSWR<IRate[]>(
    '/pubinfo?json&exchange&coursid=4',
    mockFetcher,
    {
  ...
  return error && !swrLoading ? (
    <ApiError message={error?.message || 'Unknown error'} retry={refetchData} />
  ) : (
    <Box
      sx={{
        display: 'flex',
    ...
    >
      ...
      >
        <Typography component="h1" variant={isXs ? 'h5' : 'h4'}>
          Currency exchange
        </Typography>
        <Box
          sx={{
